...
namespace InteractionSystem
{
    public partial class Interactor : Node
    {
    	...
    	[Export(PropertyHint.Range, "0.05,5,0.1")] public float LongInteractionTime { get; set; } = 0.3f;
    	...
    	[Signal] public delegate void InteractedWithInteractableEventHandler(Interactor interactable);
    	[Signal] public delegate void LongInteractedWithInteractableEventHandler(Interactor interactable);
    	[Signal] public delegate void ClosestToInteractableEventHandler(Interactor interactable);
    	[Signal] public delegate void NotClosestToInteractableEventHandler(Interactor interactable);
    	[Signal] public delegate void FocusedOnInteractableEventHandler(Interactor interactable);
    	[Signal] public delegate void UnfocusedInteractableEventHandler(Interactor interactable);
    	...
    	=> GetNodeOrNull(path) is Interactable interactable ? interactable : null;

    	protected Interactable? GetRayCastedInteractable()
    	{
    		var collider = _adapter?.GetCollider();
    		NodePath? path = null;
